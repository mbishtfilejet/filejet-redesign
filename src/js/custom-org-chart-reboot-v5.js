...
// start main > cyan color rectangle templates

OrgChart.templates.olivia = Object.assign({}, OrgChart.templates.ana);
OrgChart.templates.olivia.size = [360, 110];

OrgChart.templates.olivia.entityName = function (node, data, template, config, value) {
    return `<foreignobject data-width="270" width="270" height="${node.h - 20}" x="10" y="10">
            <div class="d-flex flex-column entityDetails">
                <span class="fs-5 font-weight-500 text-white" style="line-height: normal;">${data.Entity_Name}</span>
                <span class="fs-6 font-weight-400 text-white mt-2 lh-1">${data.entityData}</span>
            </div>
    </foreignobject>`;
};
OrgChart.templates.olivia.node = function (node, data, template, config) {
    return `<rect x="0" y="0" height="${node.h}" width="${node.w}" fill="#039BE5" stroke-width="1" stroke="#039BE5" rx="10" ry="10"></rect>`;
};

OrgChart.templates.olivia.nodeMenuButton = function (node, data, template, config) {
    return `<g style="cursor:pointer;" transform="matrix(1,0,0,1,${node.w - 25},15)">
...
OrgChart.templates.olivia.icons_0 = function (node, data, template, config, value) {
    return `<foreignobject data-width="60" class="d-block text-end" fill="#000000" width="60" height="28" x="${node.w - 68}" y="30">${value}</foreignobject>`;
}
// end olivia > cyan color rectangle templates

// 
OrgChart.templates.individualOwners = Object.assign({}, OrgChart.templates.olivia);
OrgChart.templates.individualOwners.size = [360, 110];
...
OrgChart.templates.individualOwners.entityName = function (node, data, template, config, value) {
    return `<foreignobject data-width="270" width="270" height="${node.h - 20}" x="10" y="10">
            <div class="d-flex flex-column justify-content-between entityDetails h-100">
                <div class="d-flex flex-column">
                    <span class="fs-5 font-weight-500 text-white" style="line-height: normal;">${data.Entity_Name}</span>
                    <span class="fs-6 font-weight-400 text-white mt-2 lh-1">${data.entityData}</span>
                </div>
                <div class="d-flex flex-column mt-auto">
                    <span class="fs-5 font-weight-500 text-white" style="line-height: normal;">${data.individualOwnerName}</span>
                    <span class="fs-6 font-weight-400 text-white lh-1">${data.additionalEntityData}</span>
                </div>
            </div>
    </foreignobject>`;
};
...
OrgChart.templates.individualOwners.node = function (node, data, template, config) {
    return `<rect x="0" y="0" height="${node.h}" width="${node.w}" fill="#039BE5" stroke-width="1" stroke="#039BE5" rx="10" ry="10"></rect>`;
};

OrgChart.templates.individualOwners.nodeMenuButton = function (node, data, template, config) {
    return `<g style="cursor:pointer;" transform="matrix(1,0,0,1,${node.w - 25},15)">
                <rect x="-4" y="-10" fill="#000000" fill-opacity="0" width="22" height="22"></rect>
                <circle cx="0" cy="0" r="2" fill="#FFFFFF"></circle>
                <circle cx="7" cy="0" r="2" fill="#FFFFFF"></circle>
...
OrgChart.templates.individualOwners.icons_0 = function (node, data, template, config, value) {
    return `<foreignobject data-width="60" class="d-block text-end" fill="#000000" width="60" height="28" x="${node.w - 68}" y="30">${value}</foreignobject>`;
}
// end
...
OrgChart.templates.externalIndividualOwners.size = [360, 110];
// OrgChart.templates.individualOwners.nodeMenuButton = '<g transform="matrix(1,0,0,1,280,55)" data-ctrl-n-menu-id="{id}"><rect x="-4" y="-10" fill="#fff" fill-opacity="0" width="22" height="22"></rect><circle cx="0" cy="0" r="2" fill="#fff"></circle><circle cx="7" cy="0" r="2" fill="#fff"></circle><circle cx="14" cy="0" r="2" fill="#fff"></circle></g>';
OrgChart.templates.externalIndividualOwners.entityName = function (node, data, template, config, value) {
    return `<foreignobject data-width="270" width="270" height="${node.h - 20}" x="10" y="10">
            <div class="d-flex flex-column justify-content-between entityDetails h-100">
                <div class="d-flex flex-column">
                    <span class="fs-5 font-weight-500 text-white" style="line-height: normal;">${data.Entity_Name}</span>
...
OrgChart.templates.externalIndividualOwners.node = function (node, data, template, config) {
    return `<rect x="0" y="0" height="${node.h}" width="${node.w}" fill="#F57C00" stroke-width="1" stroke="#F57C00" rx="10" ry="10"></rect>`;
};

OrgChart.templates.externalIndividualOwners.nodeMenuButton = function (node, data, template, config) {
    return `<g style="cursor:pointer;" transform="matrix(1,0,0,1,${node.w - 25},15)">
                <rect x="-4" y="-10" fill="#000000" fill-opacity="0" width="22" height="22"></rect>
                <circle cx="0" cy="0" r="2" fill="#FFFFFF"></circle>
                <circle cx="7" cy="0" r="2" fill="#FFFFFF"></circle>
...
OrgChart.templates.externalIndividualOwners.icons_0 = function (node, data, template, config, value) {
    return `<foreignobject data-width="60" class="d-block text-end" fill="#000000" width="60" height="28" x="${node.w - 68}" y="30">${value}</foreignobject>`;
}
// end 

// start ula >red rectangle templates > unmanaged entity
OrgChart.templates.ula.size = [360, 110];
...
OrgChart.templates.ula.entityName = function (node, data, template, config, value) {
    return `<foreignobject data-width="270" width="270" height="${node.h - 20}" x="10" y="10">
            <div class="d-flex flex-column justify-content-between entityDetails">
                <span class="fs-5 font-weight-500 text-white" style="line-height: normal;">${data.Entity_Name}</span>
                <span class="fs-6 font-weight-400 text-white mt-2 lh-1">${data.entityData}</span>
            </div>
    </foreignobject>`;
...
OrgChart.templates.ula.plus = '<circle cx="15" cy="15" r="15" fill="#ffffff" stroke="#aeaeae" stroke-width="1"></circle>'
    + '<text text-anchor="middle" style="font-size: 18px;cursor:pointer;" fill="#757575" x="15" y="22">{collapsed-children-count}</text>';
OrgChart.templates.ula.link = '<path stroke-linejoin="round" stroke="#000000" stroke-width="1px" fill="none" d="{rounded}" />';

OrgChart.templates.ula.icons_0 = function (node, data, template, config, value) {
    return `<foreignobject data-width="60" class="d-block text-end" fill="#000000" width="60" height="28" x="${node.w - 68}" y="30">${value}</foreignobject>`;
}
// end ula >red rectangle templates > unmanaged entity

// start polina >gary rounded shape templates > foreign entity
OrgChart.templates.polina.size = [360, 90];
...
OrgChart.templates.polina.entityName = function (node, data, template, config, value) {
    return `<foreignobject data-width="270" width="270" height="${node.h - 20}" x="25" y="10">
            <div class="d-flex flex-column justify-content-between entityDetails">
                <span class="fs-5 font-weight-500 text-white" style="line-height: normal;">${data.Entity_Name}</span>
                <span class="fs-6 font-weight-400 text-white mt-2 lh-1">${data.entityData}</span>
            </div>
...
OrgChart.templates.polina.nodeMenuButton = function (node, data, template, config) {
    return `<g style="cursor:pointer;" transform="matrix(1,0,0,1,${node.w - 50},15)">
                <rect x="-4" y="-10" fill="#000000" fill-opacity="0" width="22" height="22"></rect>
                <circle cx="0" cy="0" r="2" fill="#FFFFFF"></circle>
                <circle cx="7" cy="0" r="2" fill="#FFFFFF"></circle>
                <circle cx="14" cy="0" r="2" fill="#FFFFFF"></circle>
...
OrgChart.templates.polina.icons_0 = function (node, data, template, config, value) {
    return `<foreignobject data-width="60" class="d-block text-end" fill="#000000" width="60" height="28" x="${node.w - 68}" y="30">${value}</foreignobject>`;
}
// en

OrgChart.templates.polina.node =
    `<rect x="0" y="0" height="{h}" width="{w}" fill="#929394" stroke-width="1" stroke="#929394" rx="40" ry="40"></rect>`;
...
OrgChart.templates.additionalOwners.size = [360, 110];
OrgChart.templates.additionalOwners.entityName = '<text data-width="270" class="fs-5 font-weight-500 fst-italic text-white entityNameEllipsis" data-text-overflow="multiline" x="15" y="25" fill="#fff">{val}</text>';
OrgChart.templates.additionalOwners.entityTitle = function (node, data, template, config, value) {
    const shift = data._meta.shift
    return `<foreignobject data-width="240" data-marrk-field="EntityTitle" class="fs-9 font-weight-400 text-white" data-text-overflow="ellipsis" fill="#ffffff" width="230" height="22" x="15" y="${30 + shift}">${value}</foreignobject>`;
}

OrgChart.templates.additionalOwners.html = function (node, data, template, config, value) {
    const shift = data._meta.shift
    return `<foreignobject data-width="350" data-marrk-field="html" class="fs-7 font-weight-400 pt-0 additionalItems text-white" data-text-overflow="ellipsis" fill="#ffffff" width="340" height="54" x="15" y="${50 + shift}">${value}</foreignobject>`;
}
...
OrgChart.templates.additionalOwners.node = function (node, data, template, config) {
    const c = 20;
    const r = 10;
    return `
    <svg id="resizableSvg" width="${node.w}" height="${node.h}" xmlns="http://www.w3.org/2000/svg">
        <path
            d="
            M ${r} 0
            H ${node.w - c}
            L ${node.w} ${c}
            V ${node.h - r}
            Q ${node.w} ${node.h} ${node.w - r} ${node.h}
            H ${r}
            Q 0 ${node.h} 0 ${node.h - r}
            V ${r}
            Q 0 0 ${r} 0
            Z
            "
            fill="#48AF56" stroke="#52E34B" stroke-width="1"
...
fetch('data-reboot-v2.json')
    ...
    .then(data => {
        ...
        const updatedData = setLineShift(data)
        setTimeout(function () {
            chart.load(updatedData);
...
chart.on('node-initialized', function (sender, args) {
    ...
    if (data._meta.changeHeight) {
        node.h = node.h + data._meta.shift;
    }
});
...
function getTextWidth(text) {
    ctx.font = "500 16px / 28px Sora";
    return ctx.measureText(text).width;
}

function setLineShift(data) {

    const containerWidth = 250; // px
    const avgCharWidth = 10; // px
    const avglineHeight = 20; // px
    const offset = 5;
    const updatedData = data.map(item => {

        let text = item?.Entity_Name || item?.additionalEntityName
        ...
        if (textWidth > containerWidth) {
            let charsPerLine = Math.floor(containerWidth / avgCharWidth);
            lineCount = Math.ceil(text.length / charsPerLine);
        }

        if (item?.tags?.includes("individualOwners")
            || item?.tags?.includes("additionalOwners")
            || item?.tags?.includes("externalIndividualOwners")) {
            changeHeight = lineCount > 1;
        } else {
            changeHeight = lineCount >= 3;
        }
        ...
        let result = {
            ...
            _meta: {
                lineCount,
                shift: ((lineCount - 1) * avglineHeight),
                changeHeight
            }
        };
        return result
