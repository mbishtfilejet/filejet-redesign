...
OrgChart.templates.olivia.ownership = '<text data-width="240" class="fs-6" fill="#000000" x="15" y="90">{val}</text>';
...
chart.searchUI.on('show-items', function(sender){
    if (sender.lastSearch.length){
        for (var item of sender.lastSearch){
            var nameFieldElement = sender.instance.element.querySelector(`[data-n-id="${item.id}"] [data-marrk-field="name1"]`);
            nameFieldElement.innerHTML = item.__searchMarks;
        }
    }
    else{
